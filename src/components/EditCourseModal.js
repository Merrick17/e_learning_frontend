...
import { addCourseApi, editCourseApi } from '../redux/actions/course.actions';
import { useEffect } from 'react';
const EditCourseModal = ({ show, closeModal }) => {

    const dispatch = useDispatch();
    const categoryState = useSelector((state) => state.category)
    const userState = useSelector((state) => state.users)
    const { selectedCourseEdit } = useSelector((state) => state.courses)
    ...
    }, [userState, categoryState, selectedCourseEdit])
                                ...
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div className="mt-5">
                                        <div className="flex flex-col justify-items-center items-center">
                                            ...
                                            <div className=" relative mb-2">
                                                <input type="number" value={price} name="price" id="rounded-email" className=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Prix" onChange={(event) => {
