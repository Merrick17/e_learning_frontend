...
const AddCategModal = ({ show, closeModal }) => {
    const [name, setName] = useState('');
    const dispatch = useDispatch();

    const confirmAdd = () => {
        let body = { name: name }
        dispatch(addCategApi(body))
        closeModal();
    }
    return (
        <div>
                                    ...
                                    <div className="mt-5">
                                        <div className="flex flex-col justify-items-center items-center">
                                            ...
                                            <div className=" relative mb-2 ">
                                                <input type="text" name="name" value={name} id="rounded-email" className=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Nom de categorie" onChange={(event) => {
